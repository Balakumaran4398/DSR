...
export class DocumentsComponent implements AfterViewInit {
  ...
  getDocumentListByProjectId() {
    this.authService.getDocumentListByProjectId(this.projectid).subscribe((res: any) => {
      this.tableData = res;
      if (this.table) {
  ...
  actionFormatter(cell: any) {
    ...
    return `
      <div class="flex items-center justify-center gap-3 w-full h-full">
        <button
          class="transition-colors ${hasFile ? 'text-slate-400 hover:text-blue-600 btn-download' : 'text-slate-300 cursor-not-allowed'}"
  ...
  handleActionClick(e: any, cell: any) {
    ...
    if (target.classList.contains('btn-download')) {
      this.downloadFile(data.file_url, data.title)
  ...
  private tryDownloadFromCandidates(downloadUrls: string[], fileName: string, index = 0, lastError?: any): void {
    ...
    this.http.get(currentUrl, {
      ...
      error: (err) => {
        ...
        this.tryDownloadFromCandidates(downloadUrls, fileName, index + 1, err);
      }
    });
  }
